...
class Simulation():
    ...
    def image_simulate(self, save_image_name):
        ...
        # 모의 영상 만들기 실험 진행 
        new_img = np.zeros((self.row, self.col, self.band), dtype=np.float32) # 모의 영상의 사이즈 생성 (전체 영상 길이, 밴드) numpy float64형태 
        ...
        one_r2_array = np.zeros((self.row, self.col, self.band))
        three_r2_array = np.zeros((self.row, self.col, self.band))
            ...
            for j in range(self.col):
                if j == 0 or j == self.col - 1 or i == 0 or i == self.row - 1:
                    ...
                    for b in range(self.band):
                        ...
                        one_y_train = np.array(one_y_train, dtype=np.float32)
                        ...
                        try:
                            ...
                            one_r2_array[i, j, b] = one_r2
                            three_r2_array[i, j, b] = three_r2
        ...
        np.save('./spring_one_ad_r2.npy', one_r2_array)
        np.save('./spring_three_ad_r2.npy', three_r2_array)
if __name__ == "__main__":
    ...
    # image save path
    save_path = "./Simulation_python/115-35_new"
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    save_image_name = save_path + "/20210323_r2.tif"
